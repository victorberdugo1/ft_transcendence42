 ...
 * Todos los routes están protegidos por requireAuth (aplicado en index.js).
 *
 * GET    /api/friends          → lista de amigos aceptados + estado online
 * GET    /api/friends/requests → solicitudes pendientes recibidas
 * POST   /api/friends/:id      → enviar solicitud a user :id
 * PUT    /api/friends/:id      → aceptar solicitud de user :id
 * DELETE /api/friends/:id      → eliminar amistad / rechazar solicitud con user :id
 */
...
async function listFriends(req, res) {
    try {
        ...
        console.error('[FRIENDS] listFriends error:', err.message);
...
async function listPendingRequests(req, res) {
    try {
        ...
        console.error('[FRIENDS] listPendingRequests error:', err.message);
        res.status(500).json({ error: 'Internal error' });
    }
}
...
async function sendRequest(req, res) {
    ...
    try {
        ...
        console.error('[FRIENDS] sendRequest error:', err.message);
        res.status(500).json({ error: 'Internal error' });
    }
}
...
async function acceptRequest(req, res) {
    ...
    try {
        ...
        console.error('[FRIENDS] acceptRequest error:', err.message);
        res.status(500).json({ error: 'Internal error' });
    }
}
