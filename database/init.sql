...
UPDATE ON friendships FOR EACH ROW EXECUTE FUNCTION set_updated_at();
-- ─── Mensajes de chat ─────────────────────────────────────
CREATE TABLE IF NOT EXISTS messages (
    id SERIAL PRIMARY KEY,
    sender_id INTEGER REFERENCES users(id) ON DELETE
    SET NULL,
        receiver_id INTEGER REFERENCES users(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS matches (
    id SERIAL PRIMARY KEY,
    player1_id INTEGER REFERENCES users(id) ON DELETE
    SET NULL,
        player2_id INTEGER REFERENCES users(id) ON DELETE
    SET NULL,
        score1 INTEGER DEFAULT 0 CHECK (score1 >= 0),
        score2 INTEGER DEFAULT 0 CHECK (score2 >= 0),
        winner_id INTEGER REFERENCES users(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS tournaments (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'open' CHECK (status IN ('open', 'ongoing', 'finished')),
...
CREATE TABLE IF NOT EXISTS tournament_matches (
    id SERIAL PRIMARY KEY,
    tournament_id INTEGER NOT NULL REFERENCES tournaments(id) ON DELETE CASCADE,
    match_id INTEGER REFERENCES matches(id) ON DELETE
    SET NULL,
...
CREATE TABLE IF NOT EXISTS spectators (
    ...
    tournament_id INTEGER REFERENCES tournaments(id) ON DELETE
    SET NULL,
        mode VARCHAR(20) NOT NULL DEFAULT 'overflow' CHECK (mode IN ('overflow', 'voluntary')),
...
VALUES (
    ...
    (
        'veteran',
        'Veterano',
        'Consigue 10 victorias'
    ),
    (
        'hot_streak',
        'En racha',
        'Alcanza una racha de 3 victorias consecutivas'
    ),
    (
        'combo_master',
        'Maestro del combo',
        'Ejecuta un combo completo'
    ),
    (
        'untouchable',
        'Intocable',
        'Gana una partida sin recibir dano'
    ),
    (
        'clean_sweep',
        'Barrida perfecta',
        'Gana conservando tus 3 stocks'
    ),
    (
        'speedrunner',
        'Velocista',
        'Cierra una victoria en 45 segundos o menos'
    ),
    (
        'bracket_breaker',
        'Rompe brackets',
        'Gana un combate de torneo'
    ),
    (
        'tournament_champion',
        'Campeon del torneo',
        'Gana un torneo completo'
    ),
    (
        'social',
        'Sociable',
        'Acepta o consigue tu primera amistad'
    ) ON CONFLICT (key) DO NOTHING;
-- ─── Usuario de prueba ─────────────────────────────
